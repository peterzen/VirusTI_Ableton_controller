...
class VirusTI:
    """ A simple script to enable control over Live with the VirusTI controller """
    ...
    def __set_track_armed_led(self):
        ...
        if list(self.song().visible_tracks).count(self.__current_track) > 0:
            if self.song().view.selected_track.arm:
                ...
                index = list(all_tracks).index(self.__current_track)
                self.send_midi(SYSEX_START + (index,) + SYSEX_END)
